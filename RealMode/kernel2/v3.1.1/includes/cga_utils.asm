...
CGA_VRAM_SEG16 equ 0xb800
CGA_VRAM_BASE32 equ (CGA_VRAM_SEG16*0x10)

CGA_VRAM_SIZE_BYTES equ 0xfa0
CGA_VRAM_SIZE_CHARS equ (CGA_VRAM_SIZE_BYTES/2)

CGA_LINE_SIZE_BYTES equ 0xa0
CGA_LINE_SIZE_CHARS equ (CGA_LINE_SIZE_BYTES/2)

CGA_NLINES equ (CGA_VRAM_SIZE_CHARS/CGA_LINE_SIZE_CHARS)
...
_cga_clearscreen:
	...
	mov ax, word CGA_VRAM_SEG16
	mov es, ax
	mov di, word 0x0

	_cga_clearscreen_loop:
	cmp di, word CGA_VRAM_SIZE_BYTES
...
;FUNCTION: clear text, preserve background color in CGA mode
;args (push order): return addr (16bit)
_cga_cleartext:
	push word bp
	mov bp, sp
...
_cga_settextpos:
	...
	xor dx, dx
	mov ax, bx
	mov cx, word CGA_LINE_SIZE_BYTES
	mul word cx
	...
	cmp bx, word CGA_LINE_SIZE_CHARS
	jae _cga_settextpos_return
...
_cga_setposword:
	...
	cmp bx, word CGA_VRAM_SIZE_BYTES
...
;args (push order): enable bool value (!0 = enable | 0 = disable) (16bit), return addr (16bit)
_cga_setblinkenable:
	push word bp
	mov bp, sp
...
_cga_addnewline:
	...
	sub bx, dx
	add bx, word CGA_LINE_SIZE_BYTES
...
_cga_printtext:
	...
	mov si, word _cga_npos
	mov cx, word [ds:si]
	cmp cx, word CGA_VRAM_SIZE_BYTES
...
;FUNCTION: print a single character at cursor position
;args (push order): character (16bit), return addr (16bit)
_cga_printchar:
	push word bp
	mov bp, sp
...
;FUNCTION: clear previous character from screen, return cursor position by 1 character
;args (push order): return addr (16bit)
_cga_clearprevchar:
	push word bp
	mov bp, sp
